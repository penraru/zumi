...
Insert the miniSD card into the computer.
Go to the boot folder which should appear in your file explorer. Alternatively you can navigate to the boot directory by entering the following into terminal:

cd ~/Volumes/boot

Create a file in this directory called wpa_supplicant.conf. The file should contain the following text:
...
The next step is to boot the Pi and test, while the SD card is still in your computer. If you're going to try to connect via SSH, it may need to be enabled.
You can connect to Linky by either using SSH (Secure Shell) or a VNC (Virtual Network Computing) Server.
First, ensure that the Linky is connected to the same Wifi Network as you. To do this, you can enter the following into your terminal:
ping linky.local
